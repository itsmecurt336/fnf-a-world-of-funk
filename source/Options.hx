...
class RobeatsChaining extends Option
{
	public function new(desc:String)
	{
		super();
	...
	{
		trace("switch");
		if (PlayState.RobeatsChaining = false)
			PlayState.RobeatsChaining = true;
		else
			PlayState.RobeatsChaining = false;

		FlxG.save.data.RobeatsChaining = PlayState.RobeatsChaining = true ? false : true;
		display = updateDisplay();
		return true;
	}
	...
	private override function updateDisplay():String
	{
		return (FlxG.save.data.RobeatsChaining == 0 ? "Chain" : "Combo");
	}
}
