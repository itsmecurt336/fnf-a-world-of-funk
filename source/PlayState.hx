...
class PlayState extends MusicBeatState
{
	...
	public static var RobeatsChaining:Bool = false;
	...
	override public function create()
	{
		...
		judgementCount = new FlxText(4, 4, 0, " ", 8, true);
		judgementCount.setFormat(Paths.font("vcr.ttf"), 16, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		judgementCount.scrollFactor.set();
		judgementCount.screenCenter(Y);
		judgementCount.x = 4;
		judgementCount.cameras = [camHUD];
		add(judgementCount);
		...
		kadeEngineWatermark.y = 0;
		add(kadeEngineWatermark);

		kadeEngineVersion = new FlxText(4, 20, 0, (Main.watermarks ? "Sagittarius Engine " + MainMenuState.kadeEngineVer : ""), 16);
		kadeEngineVersion.setFormat(Paths.font("vcr.ttf"), 16, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		kadeEngineVersion.scrollFactor.set();
		kadeEngineVersion.cameras = [camHUD];
		add(kadeEngineVersion);
	...
	override public function update(elapsed:Float)
	{
		...
		if (FlxG.keys.justPressed.SEVEN)
		{
			...
			cannotDie = true;
			#if windows
			DiscordClient.changePresence("Editing a Chart", null, null, true);
			#end
		...
		if (health <= 0 && !cannotDie && FreeplayState.curDifficulty != 3)
		{
			if (!usedTimeTravel)
			{
	...
	private function popUpScore(daNote:Note):Void
	{
		...
		if (daRating != 'shit' || daRating != 'bad')
		{
			...
			comboSpr.velocity.x += FlxG.random.int(1, 75);
	...
	{
		if (!boyfriend.stunned) // * Miss mechanic
		{
			// health -= 0.2;
			if (combo > 20 && gf.animOffsets.exists('sad'))
			{
				gf.playAnim('sad');
				FlxG.camera.flash(0xF20000, 0.2);
			}
			if (RobeatsChaining = false && combo > 0)
				combo = 0;
			else if (combo > 0 && RobeatsChaining == true)
				combo -= 20;
			else
				combo = 0;
			misses++;
